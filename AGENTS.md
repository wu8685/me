...
A Codex plugin that gives any workspace knowledge management capabilities. When installed, it provides skills, hooks, and templates for a three-layer knowledge flow system (raw → practices → cognition). The target workspace doubles as an Obsidian vault and git repo — same directory, no sync. Built for personal use, evolving toward automated knowledge processing and eventually a digital twin.
